    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(pretrained, str):
            if not quant_config and not quantized:
                self._create_model(
        ...
        **kwargs,
        ...
        if not autogptq and not gptqmodel:
            ...
            if not quant_config and not quantized:
                ...
                from ..quantized.inference import load_quantized_model
                kwargs = {"device_map": 'auto',"torch_dtype": "torch.float16"}
                self._model = load_quantized_model(pretrained, kwargs)
            else:
                from transformers import HqqConfig
                import pickle
                hqq_config_dict = pickle.load(open(quant_config,'rb'))
                hqq_config = HqqConfig(dynamic_config=hqq_config_dict)
                self._model = self.AUTO_MODEL_CLASS.from_pretrained(
                    pretrained,
                    revision=revision,
